                            ...
                            <div class="row h-100">
                                <div class="col-7 h-100">
                                    <div class="tool-bar">
                                        <div class="text-right align-items-center">
                                            <span v-if="isLoadingDownload"
                                                class="spinner-border spinner-border-sm text-dark mr-2" role="status">
                                                <span class="sr-only">Loading...</span>
                                            </span>
                                            <span v-else>
                                                <a role="button" class="h5" @click="download(file)"><i
                                    ...
                                    <div :class="file.tokenURI !== undefined &&
                                        ...
                                        <iframe v-if="link.length" style="overflow: hidden;" width="100%" height="100%"
                                            class="frame" :src="link" frameborder="0"></iframe>
                                ...
                                <div class="col-5 h-100">
                                    <v-tabs icons-and-text class="h-100" fixed-tabs grow v-model="active" color="#f7f7f7">
                                        ...
                                        <v-tab class="file-detail-tab"
                                            ...
                                            <span>Document Details</span><span class="material-icons">description
                                            </span>
                                        </v-tab>
                                        <v-tab class="file-detail-tab"
                                            active-class="tab-active text-primary font-weight-bold" :key="1">
                                            <span>Comments</span><span class="material-icons">comment </span>
                                        </v-tab>
                                        <v-tab class="file-detail-tab"
                                            active-class="tab-active text-primary font-weight-bold" :key="2">
                                            <span>Status</span>
                                                ...
                                                <div class="d-flex action">
                                                        ...
                                                        <div class="align-self-center">
                                                            <v-menu transition="slide-y-transition" bottom
                                                                ...
                                                                <div @click.stop="" class="attachment">
                                                                    <treeselect v-model="attachments" :multiple="true"
                                                                        :options="documentState.treeFolder['my-folder']"
                                                    ...
                                                    <div class="p-2">
                                                        <div class="align-self-center ml-auto ">
                                                            <v-btn v-if="!documentState.file.isLoadingComment"
                                                                style="float: right;" flat icon color="indigo"
                                                                @click="handleAddComment">
                                                                <v-icon data-toggle="tooltip" title="Send" color="blue">
                                                                    send
                                                                </v-icon>
                                                            </v-btn>
                                            ...
                                            <div v-if="!documentState.fileStatusList?.isLoading">
                                                <div v-if="fileStatus?.status">
                                                    ...
                                                    <div class="card border border-muted">
                                                        <div class="card-body">
                                                            <span>Status: &nbsp;</span>
                                                            <span v-if="fileStatus?.status === 'waiting-to-review'"
                                                                class="font-weight-bold text-warning">Waiting to
                                                                review</span>
                                                            <span v-if="fileStatus?.status === 'waiting-to-sign'"
                                                                class="font-weight-bold text-info">Waiting to
                                                                sign</span>
                                                            <span v-if="fileStatus?.status === 'rejected'"
                                                                class="font-weight-bold text-danger">Rejected</span>
                                                            <span v-if="fileStatus?.status === 'signed'"
                                                                class="font-weight-bold text-success">Signed</span>
                                                            <p></p>
                                                            <p class="font-weight-bold">Review History: </p>
                                                            <p v-for="(reviewer, index) in fileStatus?.statusDetail?.reviewerList"
                                                                :key="index + reviewer?.name">{{ reviewer?.name }}: &nbsp;
                                                                <span v-if="reviewer?.status === 'not-yet-reviewed'"
                                                                    class="font-weight-bold text-muted">Not reviewed
                                                                    yet</span>
                                                                <span v-if="reviewer?.status === 'reviewed'"
                                                                    class="font-weight-bold text-success">Reviewed at
                                                                    {{ formatDateTime(reviewer?.time) }}</span>
                                                                <span v-if="reviewer?.status === 'rejected'"
                                                                    class="font-weight-bold text-danger">Rejected at
                                                                    {{ formatDateTime(reviewer?.time) }}</span>
                                                            </p>
                                                            <p class="font-weight-bold">Sign History: </p>
                                                            <p v-for="(signer, index) in fileStatus?.statusDetail?.signerList"
                                                                :key="index + signer?.name">{{ signer?.name }}: &nbsp;
                                                                <span v-if="signer?.status === 'not-yet-signed'"
                                                                    class="font-weight-bold text-muted">Not signed
                                                                    yet</span>
                                                                <span v-if="signer?.status === 'signed'"
                                                                    class="font-weight-bold text-info">Signed at
                                                                    {{ formatDateTime(signer?.time) }}</span>
                                                                <span v-if="signer?.status === 'rejected'"
                                                                    class="font-weight-bold text-danger">Rejected at
                                                                    {{ formatDateTime(signer?.time) }}</span>
                                                            </p>
                                                        </div>
                                                            ...
                                                            <span class="float-md-right">
                                                                ...
                                                                <span v-else>
                                                                    ...
                                                                    <button v-else-if="fileStatus?.canReview"
                                                                        ...
                                                                        aria-disabled="false"
                                                                        @click="handleReject('review')">Reject</button>
                                                                </span>
                                                                <button
                                                                ...
                                                                <span v-else>
                                                                    <button v-if="fileStatus?.canReview"
                                                                        class="btn btn-warning btn-lg" role="button"
                                                                        aria-disabled="false"
                                                                        @click="handleSign('review')">Review</button>
                                                                    <button v-if="fileStatus?.canSign"
                                                                        class="btn btn-success btn-lg" role="button"
                                                                        aria-disabled="false"
                                                                        @click="handleSign('sign')">Sign</button>
                                                                </span>
                                                            </span>
                                                        </div>
                                                    </div>
        ...
        async initData(fileId) {
            ...
            try {
                ...
                if (this.file.hash) {
                    ...
                    let res = await ipfs.get(this.file.hash)
                    console.log("res", res);
