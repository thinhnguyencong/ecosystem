    ...
    <div class="comment-widgets m-b-20">
        <div v-for="(comment, index) in comments" :key="index" class="d-flex flex-row comment-row">
            ...
            <div class="p-2 comment-text w-100">
                ...
                <a href="#" @click="handleShow(comment)">X Attachment(s)</a>
...
export default {
    ...
    mounted() {
        this.$set(this, 'comments', this.commentsProp.map(x=> ({...x, isActive: false})))
    },
    watch: { 
        commentsProp: function(newVal, oldVal) { // watch it
            this.$set(this, 'comments', newVal.map(x=> ({...x, isActive: false})))
        },
    }
}
</script>
